...
class Job:
    ...
    end_frame_image_saved: bool = (
        False  # Flag to track if end frame image has been saved
    )

    def __post_init__(self):
        # Store generation type
        self.generation_type = self.params.get(
            "model_type", "Original"
        )  # Initialize generation_type
...
class VideoJobQueue:
    ...
    def __init__(self):
        ...
        self.worker_function: Callable[..., None] | None = (
            None  # Will be set from outside
        )
        ...
        try:
            ...
            if "selected_loras" in job.params and job.params["selected_loras"]:
                ...
                if not isinstance(selected_loras, list):
                    selected_loras = (
                        [selected_loras] if selected_loras is not None else []
                    )
                ...
                if not isinstance(lora_loaded_names, list):
                    lora_loaded_names = (
                        [lora_loaded_names] if lora_loaded_names is not None else []
                    )
                ...
                for lora_name in selected_loras:
                    try:
                        ...
                        if isinstance(weight, list):
                            weight_value = (
                                weight[0] if weight and len(weight) > 0 else 1.0
                            )
                        else:
                            weight_value = weight
                        # Store as float
        ...
        try:
            ...
            for job_id in job_ids:
                ...
                if job:
                    ...
                    if (
                        "input_image" in job.params
                        and isinstance(job.params["input_image"], np.ndarray)
                        and not job.input_image_saved
                    ):
                        input_image_path = os.path.join(
                            queue_images_dir, f"{job_id}_input.png"
                        )
                        try:
                    ...
                    if (
                        "end_frame_image" in job.params
                        and isinstance(job.params["end_frame_image"], np.ndarray)
                        and not job.end_frame_image_saved
                    ):
                        end_frame_image_path = os.path.join(
                            queue_images_dir, f"{job_id}_end_frame.png"
                        )
                        try:
                ...
                if job:
                    ...
                    try:
                        ...
                        metadata = create_metadata(
                            job.params, job.id, settings.settings
                        )
                        ...
                        if job.end_frame_image_saved:
                            ...
                            if os.path.exists(end_frame_image_path):
                                metadata["saved_end_frame_image_path"] = (
                                    end_frame_image_path
                                )
    ...
    def cleanup_orphaned_videos(self, current_job_ids_uuids):  # Renamed arg for clarity
        ...
        try:
            ...
            with self.lock:  # Access self.jobs safely
                for (
                    job_id_uuid
                ) in current_job_ids_uuids:  # Iterate using the provided UUIDs
            ...
            for filename in os.listdir(input_files_dir):
                if filename.endswith(".mp4"):  # Only process MP4 files
                    file_path_to_check = os.path.normpath(
                        os.path.join(input_files_dir, filename)
                    )
        ...
        try:
            ...
            for filename in os.listdir(queue_images_dir):
                ...
                if filename.endswith(".png") and (
                    "_input.png" in filename or "_end_frame.png" in filename
                ):
    ...
    def add_job(
        ...
        with self.lock:
            ...
            self.queue.put(
                job_id
            )  # Only the parent (or single) job is added to the queue initially
    ...
    def get_queue_position(self, job_id):
        ...
        with self.lock:
            ...
            for j in self.jobs.values():
                if j.status == JobStatus.PENDING and j.created_at < job.created_at:
                    position += 1
            return position
            ...
            with self.lock:
                for job_id, job_data in serialized_jobs.items():
                    ...
                    params = {
                        ...
                        "magcache_retention_ratio": job_data.get(
                        ...
                        "combine_with_source": job_data.get(
                            "combine_with_source", False
                        ),
                    }

                    # Load input image from disk if saved path exists
                    if "saved_input_image_path" in job_data and os.path.exists(
                        job_data["saved_input_image_path"]
                    ):
                        try:
                            ...
                            params["input_image_path"] = (
                                input_image_path  # Store the path for reference
                            )
                    ...
                    if isinstance(input_video_val, str):  # Check if it's a string path
                        if os.path.exists(
                            input_video_val
                        ):  # Now it's safe to call os.path.exists
                            try:
                                video_path = (
                    ...
                    if "saved_end_frame_image_path" in job_data and os.path.exists(
                        job_data["saved_end_frame_image_path"]
                    ):
                        try:
                            ...
                            params["end_frame_image_path"] = (
                                end_frame_image_path  # Store the path for reference
                            )
                            # Make sure end_frame_strength is set if this is an endframe model
                            if (
                                params["model_type"] == "Original with Endframe"
                                or params["model_type"] == "F1 with Endframe"
                            ):
                                if (
                                    "end_frame_strength" not in params
                                    or params["end_frame_strength"] is None
                                ):
                    ...
                    if "loras" in job_data:
                        ...
                        if os.path.isdir(lora_dir):
                            for root, _, files in os.walk(lora_dir):
                                for file in files:
                                    if file.endswith(".safetensors") or file.endswith(
                                        ".pt"
                                    ):
                                        lora_relative_path = os.path.relpath(
                                            os.path.join(root, file), lora_dir
                                        )
                                        lora_name = str(
                                            PurePath(lora_relative_path).with_suffix("")
                                        )
                        ...
                        combined_lora_names = list(
                            set(current_lora_names + selected_loras)
                        )
                    ...
                    job = Job(
                        ...
                        input_image_saved="saved_input_image_path" in job_data
                        and os.path.exists(job_data["saved_input_image_path"]),
                        end_frame_image_saved="saved_end_frame_image_path" in job_data
                        and os.path.exists(job_data["saved_end_frame_image_path"]),
                    )
        ...
        try:
            ...
            try:
                ...
                for job_id, job_data in queue_data.items():
                    ...
                    input_image_path = os.path.join(
                        target_queue_images_dir, input_image_filename
                    )
                    end_frame_image_path = os.path.join(
                        target_queue_images_dir, end_frame_image_filename
                    )
                    ...
                    if os.path.exists(input_image_path):
                        ...
                        job_data["saved_input_image_path"] = os.path.join(
                    ...
                    if os.path.exists(end_frame_image_path):
                        ...
                        # Fallback to updating the existing path
                        job_data["saved_end_frame_image_path"] = os.path.join(
                            target_queue_images_dir,
                    ...
                    # If input_video is None, but input_image_path is a video path (for Video/Video F1 models)
                        ...
                        current_input_video is None
                        and isinstance(current_input_image_path, str)
                        and model_type_for_job in ("Video", "Video F1")
                        and current_input_image_path.lower().endswith(video_extensions)
                    ):
            ...
            try:
                ...
                with self.lock:
                    ...
                    if previously_running_job:
                        ...
                        job.stream.output_queue.push(
                            (
                                "progress",
                                (
                                    None,
                                    "",
                                    make_progress_bar_html(0, "Resuming job..."),
                                ),
                            )
                        )
                ...
                try:
                    if self.worker_function is None:
                        raise ValueError(
                            "Worker function not set. Call set_worker_function() first."
                        )
    ...
    def _check_and_process_completed_grids(self):
        ...
        with self.lock:
            ...
            running_grid_jobs = [
                ...
                if job.job_type == JobType.GRID and job.status == JobStatus.RUNNING
            ]

            for grid_job in running_grid_jobs:
                # Check if all child jobs are completed
                child_jobs = [
                    self.jobs.get(child_id) for child_id in grid_job.child_job_ids
                ]
                ...
                if all_children_done:
                    ...
                    child_results = [
                        ...
                        if child.status == JobStatus.COMPLETED and child.result
                    ]
                    ...
                    try:
                        ...
                        grid_filename = os.path.join(
                            output_dir, f"grid_{grid_job.id}.txt"
                        )
