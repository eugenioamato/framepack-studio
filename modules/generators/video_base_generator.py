...
class VideoBaseModelGenerator(BaseModelGenerator):
    ...
    def min_real_frames_to_encode(self, real_frames_available_count):
        ...
        max_real_frames_used_for_context = (
            max_latents_used_for_context * latent_size_factor
        )

        # Shortest of available frames and max frames used for context
        trimmed_real_frames_count = min(
    ...
    def extract_video_frames(
        ...
        try:
            ...
            frame_indices_to_extract = range(
                start_frame_index, total_frames_in_video_file
            )
            frames = vr.get_batch(
            ...
            for i, frame in tqdm(
                ...
                total=num_real_frames,
                mininterval=0.1,
            ):
                frame_np = resize_and_center_crop(
                    frame, target_width=target_width, target_height=target_height
                )
    ...
    def video_encode(
        ...
        input_frames_resized_np, fps, target_height, target_width = (
            self.extract_video_frames(
                encoding, video_path, resolution, no_resize, input_files_dir
            )
        )
        ...
        try:
            ...
            frames_pt = frames_pt.permute(
                0, 3, 1, 2
            )  # Shape: (num_real_frames, channels, height, width)
            frames_pt = frames_pt.unsqueeze(
                0
            )  # Shape: (1, num_real_frames, channels, height, width)
            frames_pt = frames_pt.permute(
                0, 2, 1, 3, 4
            ...
            with torch.no_grad():
                ...
                for i in tqdm(
                    range(0, frame_count, vae_batch_size),
    ...
    def combine_videos(self, source_video_path, generated_video_path, output_path):
        ...
        try:
            ...
            filter_script_path = os.path.join(
                temp_dir, f"filter_script_{os.path.basename(output_path)}.txt"
            )
            ...
            generated_target_width = int(
                generated_width * (target_height / generated_height)
            )
