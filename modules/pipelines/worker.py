...
def get_cached_or_encode_prompt(
    ...
    if prompt in prompt_embedding_cache:
        ...
        llama_attention_mask = (
            llama_mask_cpu.to(target_device) if llama_mask_cpu is not None else None
        )
...
def worker(
    ...
    if isinstance(selected_loras, list):
        actual_selected_loras_for_worker = [
            lora for lora in selected_loras if lora != DUMMY_LORA_NAME
        ]
        if (
            DUMMY_LORA_NAME in selected_loras
            and DUMMY_LORA_NAME in actual_selected_loras_for_worker
        ):  # Should not happen if filter works
            logger.error(
    ...
    total_steps = (
        total_latent_sections * steps
    )  # Total diffusion steps over all segments
    ...
    if job_stream is not None:
        try:
            ...
            logger.error(f"Error storing initial progress data: {e}")
    ...
    try:
        ...
        if not high_vram:
            ...
            if (
                studio_module.current_generator is not None
                and studio_module.current_generator.transformer is not None
            ):
                offload_model_from_device_for_memory_preservation(
        ...
        force_new_generator = studio_module.is_reload_required(
        ...
        new_generator = (
            ...
            if force_new_generator
            else None
        )  # settings is now a singleton and the Base Model is setup to get the instance if this is not provided - candidate for removal
        ...
        assert studio_module.current_generator is not None, (
            "current_generator should not be None after model assignment"
        )

        # Ensure the transformer is loaded
        assert studio_module.current_generator.transformer is not None, (
            "current_generator.transformer should not be None after model assignment. load_model() must be called once."
        )
        ...
        studio_module.update_model_state(
            selected_loras=selected_loras,
            lora_values=lora_values,
            lora_loaded_names=lora_loaded_names,
        )
        ...
        stream_to_use.output_queue.push(
            (
                "progress",
                (None, "", make_progress_bar_html(0, "Preprocessing inputs...")),
            )
        )
        ...
        stream_to_use.output_queue.push(
            (
                "progress",
                (None, "", make_progress_bar_html(0, "Text encoding all prompts...")),
            )
        )
        ...
        end_of_input_video_embedding = (
            None  # Video model end frame CLIP Vision embedding
        )
        ...
        if model_type == "Video" or model_type == "Video F1":
            stream_to_use.output_queue.push(
                (
                    "progress",
                    (None, "", make_progress_bar_html(0, "Video processing ...")),
                )
            )
            ...
            (
                ...
                width,
                input_video_pixels,
                end_of_input_video_image_np,
                input_frames_resized_np,
            ) = studio_module.current_generator.video_encode(
                video_path=job_params[
                    "input_image"
                ],  # For Video model, input_image contains the video path
            ...
            if end_of_input_video_image_np is not None:
                try:
                    ...
                    save_last_video_frame(
                        job_params, job_id, settings, end_of_input_video_image_np
                    )
                except Exception as e:
                    logger.error(f"Error saving last video frame: {e}")
                    logger.error(traceback.format_exc())
            ...
            end_of_input_video_embedding = hf_clip_vision_encode(
                end_of_input_video_image_np, feature_extractor, image_encoder
            ).last_hidden_state
            ...
            if not has_input_image and job_params.get("latent_type") == "Noise":
                ...
                image_encoder_output = hf_clip_vision_encode(
                    black_image_np, feature_extractor, image_encoder
                )
                image_encoder_last_hidden_state = image_encoder_output.last_hidden_state
        ...
        if (
            ...
            if not isinstance(end_frame_image, np.ndarray):
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not convert end_frame_image to numpy array: {e_conv}. Skipping end frame."
                    )
            ...
            if end_frame_image is not None:
                ...
                end_frame_output_dimensions_np = resize_and_center_crop(
                    end_frame_np, width, height
                )
                end_frame_output_dimensions_pt = (
                    torch.from_numpy(end_frame_output_dimensions_np).float() / 127.5 - 1
                )
                end_frame_output_dimensions_pt = end_frame_output_dimensions_pt.permute(
                    2, 0, 1
                )[None, :, None]  # VAE expects [B, C, F, H, W]
                end_frame_output_dimensions_latent = vae_encode(
                ...
                if model_type == "Video":
                    if (
                        not high_vram
                    ):  # Ensure image_encoder is on GPU for this operation
                        load_model_as_complete(image_encoder, target_device=gpu)
                    from diffusers_helper.clip_vision import hf_clip_vision_encode

                    end_clip_embedding = hf_clip_vision_encode(
                        end_frame_np, feature_extractor, image_encoder
                    ).last_hidden_state
                    end_clip_embedding = end_clip_embedding.to(
                        studio_module.current_generator.transformer.dtype
                    )
        ...
        for prompt_key in encoded_prompts:
            ...
            clip_l_pooler = clip_l_pooler.to(
                studio_module.current_generator.transformer.dtype
            )
        ...
        clip_l_pooler_n = clip_l_pooler_n.to(
            studio_module.current_generator.transformer.dtype
        )
        ...
        if (
            model_type != "Video" and model_type != "Video F1"
        ):  # Skip for Video models as we already initialized it
            history_latents = studio_module.current_generator.prepare_history_latents(
                height, width
            )
            ...
            if model_type == "F1":
                history_latents = (
                    studio_module.current_generator.initialize_with_start_latent(
        ...
        latent_paddings = studio_module.current_generator.get_latent_paddings(
                ...
                (
                    ...
                    (
                        ...
                        make_progress_bar_html(
                            0,
                            f"Loading LoRA{'s' if len(selected_loras) > 1 else ''} ...",
                        ),
                    ),
                )
            )
            lora_folder_from_settings = settings.get("lora_dir")
            studio_module.current_generator.load_loras(
                selected_loras,
                lora_folder_from_settings,
                lora_loaded_names,
                lora_values,
            )
        ...
        def callback(d):
            ...
            if studio_module.current_generator is None:
                logger.warning(
                    "Worker callback: current_generator is None, cannot process progress update."
                )
                return
            ...
            preview = (
                (preview * 255.0).detach().cpu().numpy().clip(0, 255).astype(np.uint8)
            )
            ...
            segment_hint = (
                f"Sampling {current_step}/{steps}  ETA {fmt_eta(segment_eta)}"
            )
            total_hint = (
                f"Total {total_steps_done}/{total_steps}  ETA {fmt_eta(total_eta)}"
            )
            ...
            if model_type == "Video":
                ...
                current_pos = (
                    input_video_time + (total_generated_latent_frames * 4 - 3) / 30
                )
                # Original position is the remaining time to generate
                original_pos = (
                    total_second_length - (total_generated_latent_frames * 4 - 3) / 30
                )
            else:
            ...
            progress_data = {
                "preview": preview,
                "desc": desc,
                "html": make_progress_bar_html(percentage, segment_hint)
                + make_progress_bar_html(total_percentage, total_hint),
            }
            ...
            if job_stream is not None:
                try:
                    ...
                    logger.error(f"Error updating job progress data: {e}")
            ...
            stream_to_use.output_queue.push(
                (
                    ...
                    (
                        preview,
                        desc,
                        make_progress_bar_html(percentage, segment_hint)
                        + make_progress_bar_html(total_percentage, total_hint),
                    ),
                )
            )
        ...
        for i_section_loop, latent_padding in enumerate(
            latent_paddings
            ...
            if (
                blend_sections_int > 0
                and prompt_change_indices
                and len(prompt_sections) > 1
            ):
            ...
            if blend_alpha is not None and prev_prompt != next_prompt:
                # Blend embeddings
                prev_llama_vec, prev_llama_attention_mask, prev_clip_l_pooler = (
                    encoded_prompts[prev_prompt]
                )
                next_llama_vec, next_llama_attention_mask, next_clip_l_pooler = (
                    encoded_prompts[next_prompt]
                )
                llama_vec = (
                    1 - blend_alpha
                ) * prev_llama_vec + blend_alpha * next_llama_vec
                ...
                llama_vec, llama_attention_mask, clip_l_pooler = encoded_prompts[
                    current_prompt
                ]
            ...
            if (
                ...
                if end_frame_strength != 1.0:  # Only multiply if not full strength
                    actual_end_frame_latent_for_history = (
                        actual_end_frame_latent_for_history * end_frame_strength
                    )
                ...
                if (
                    history_latents.shape[2] >= 1
                ):  # Check if the 'Depth_slots' dimension is sufficient
                    if model_type == "Original with Endframe":
                        # For Original model, apply to the beginning (position 0)
                        history_latents[:, :, 0:1, :, :] = (
                            actual_end_frame_latent_for_history.to(
            ...
            if model_type == "Video":
                ...
                (
                    ...
                    clean_latents_2x,
                    clean_latents_4x,
                ) = studio_module.current_generator.video_prepare_clean_latents_and_indices(
                    end_frame_output_dimensions_latent,
                    end_frame_strength,
                    end_clip_embedding,
                    end_of_input_video_embedding,
                    latent_paddings,
                ...
                (
                    ...
                    clean_latent_2x_indices,
                    clean_latent_4x_indices,
                ) = studio_module.current_generator.prepare_indices(
                ...
                clean_latents, clean_latents_2x, clean_latents_4x = (
                    studio_module.current_generator.prepare_clean_latents(
                ...
                stream_to_use.output_queue.push(
                    (
                        ...
                        (
                            None,
                            "",
                            make_progress_bar_html(0, "Moving model to GPU ..."),
                        ),
                    )
                )
                move_model_to_device_with_memory_preservation(
            ...
            if (
                model_type in ("Original", "Original with Endframe")
                and has_input_image
                and is_last_section
            ):
            ...
            history_latents = studio_module.current_generator.update_history_latents(
            ...
            real_history_latents = (
                studio_module.current_generator.get_real_history_latents(
            ...
            if history_pixels is None:
                ...
                current_pixels = studio_module.current_generator.get_current_pixels(
                    real_history_latents, section_latent_frames, vae
                )

                # Update history pixels using the generator
                history_pixels = studio_module.current_generator.update_history_pixels(
        ...
        stream_to_use.output_queue.push(
            ("error", f"Error during generation: {traceback.format_exc()}")
        )
        if not high_vram:
                ...
                if video_files:
                    ...
                    def get_frame_count(filename):
                        try:
                            ...
                            return int(
                                filename.replace(f"{job_id}_", "").replace(".mp4", "")
                            )
                        except Exception:
                            return -1
        ...
        if (
            combine_v1
            and (model_type == "Video" or model_type == "Video F1")
            ...
            try:
                ...
                if input_video_path and os.path.exists(input_video_path):
                    final_video_path_for_combine = (
                        None  # Use a different variable name to avoid conflict
                    )
                    ...
                    if video_files_for_combine:
                        ...
                        def get_frame_count_for_combine(
                            filename,
                        ):  # Renamed to avoid conflict
                            try:
                                return int(
                                    filename.replace(f"{job_id}_", "").replace(
                                        ".mp4", ""
                                    )
                                )
                            except Exception:
                        ...
                        if (
                            video_files_sorted_for_combine
                        ):  # Check if the list is not empty
                            final_video_path_for_combine = os.path.join(
                                output_dir, video_files_sorted_for_combine[-1]
                            )

                    if final_video_path_for_combine and os.path.exists(
                        final_video_path_for_combine
                    ):
                        combined_output_filename = os.path.join(
                            output_dir, f"{job_id}_combined_v1.mp4"
                        )
                        ...
                        try:
                            if hasattr(
                                ...
                                if combined_result:
                                    ...
                                    logger.warning(
                                        "Failed to create combined video, falling back to direct ffmpeg method"
                                    )
                                    combined_result = None
                            else:
                                logger.warning(
                                    "VideoModelGenerator does not have combine_videos method. Using fallback method."
                                )
                        except (
                            Exception
                        ) as e_combine:  # Use a different exception variable name
                            logger.error(f"Error in combine_videos method: {e_combine}")
                            logger.warning("Falling back to direct ffmpeg method")
                            combined_result = None
                        ...
                        if not combined_result:
                            ...
                            video_processor_settings = (
                                settings.settings
                                if hasattr(settings, "settings")
                                else settings
                            )
                            video_processor = VideoProcessor(
                                message_manager, video_processor_settings
                            )
                            ...
                            if ffmpeg_exe:
                                ...
                                temp_list_file = os.path.join(
                                    output_dir, f"{job_id}_filelist.txt"
                                )
                                with open(temp_list_file, "w") as f:
                                    f.write(f"file '{input_video_path}'\n")
                                    f.write(
                                        f"file '{final_video_path_for_combine}'\n"
                                ...
                                stream_to_use.output_queue.push(
                                    ("file", combined_output_filename)
                                )
                            else:
                                logger.error(
                                    "FFmpeg executable not found. Cannot combine videos."
                                )
                    else:
                        logger.warning(
                            f"Final video not found for combining with source: {final_video_path_for_combine}"
                        )
                else:
                    logger.warning(f"Input video path not found: {input_video_path}")
            except (
                Exception
            ) as e_combine_outer:  # Use a different exception variable name
                logger.error(f"Error combining videos: {e_combine_outer}")
                logger.error(traceback.format_exc())
        ...
        if (
            (model_type == "Video" or model_type == "Video F1")
            and combine_with_source
            ...
            try:
                ...
                input_frames_resized_np, fps, target_height, target_width = (
                    studio_module.current_generator.extract_video_frames(
                        is_for_encode=False,
                ...
                if (
                    input_frames_resized_np is not None and history_pixels.numel() > 0
                ):  # Check if history_pixels is not empty
                    combined_sequential_output_filename = os.path.join(
                        output_dir, f"{job_id}_combined.mp4"
                    )
                    ...
                    combined_sequential_result_path = (
                        combine_videos_sequentially_from_tensors(
